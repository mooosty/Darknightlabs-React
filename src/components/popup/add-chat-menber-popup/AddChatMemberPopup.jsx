...
const AddChatMemberPopup = ({ chatId, open, handleClose }) => {
  ...
  const handleAddmember = () => {
    ...
    setIsLoading(true);
    Promise.allSettled(responseArr)
      .then(() => {
        setSelectedMember([]);
        handleClose();
      })
  ...
  useEffect(() => {
    if (searchUser !== "") handleSearch(searchUser);
  }, [initialMember]);
  ...
  return (
    <>
      <div className={`model_bg ${open ? "active" : ""} bg-red-soft`} style={{}}>
        <div className={`add_chatMember_popup`}>
          <div className="model_box" ref={closeRef}>
                          ...
                          <div className="item_left">
                            <div className="costum_checkbox">
                              <input
                                ...
                                defaultChecked={selectedMember.includes(data["_id"])}
                                onClick={() => handleSelcteMember(data["_id"])}
                              />
                              <label htmlFor={`checkBox_${index}`} className="costum_checkbox_label"></label>
                            </div>
                            <div className="item_name">
                              <div className="user_name">{data.name[0]?.toUpperCase()}</div>
                              {/* <div className="image">
                                <img src={member} alt="" />
                              </div> */}
                              <div className="name">
            ...
            <div className="model_footer">
              ...
              <button className="add_btn" onClick={handleAddmember} disabled={isLoading}>
                        ...
                        <div key={index} className="member_list_item">
                          <div className="item_left">
                            <div className="costum_checkboxs">
                              <input
                                type="checkbox"
                                id={`add_member_checkbox_${index}`}
                                defaultChecked={selectedMember.includes(data["_id"])}
                                name="add_member_checkbox"
                                className="costum_checkbox_inputs"
                                onClick={() => handleSelcteMember(data["_id"])}
                              />

                              <label
                                htmlFor={`add_member_checkbox_${index}`}
                                className="costum_checkbox_labels"
                              ></label>
                            </div>
                            <div className="item_name">
                              <div className="user_name">{data.name[0]?.toUpperCase()}</div>
